...
func createCacheFacade(ctx context.Context, client client.Client, config *config.Config, tracker convtypes.Tracker, sslCerts *SSL, dynconfig *convtypes.DynamicConfig, status svcStatusUpdateFnc) convtypes.Cache {
	return &c{
		ctx:       ctx,
...
func (c *c) isValidGatewayA1(gw *gatewayv1alpha1.Gateway) bool {
	...
	if err != nil {
		c.log.Error(err, "error reading GatewayClass v1alpha1", "classname", className)
		return false
	} else if gwClass == nil {
...
func (c *c) getGatewayClassA1(className string) (*gatewayv1alpha1.GatewayClass, error) {
	if !c.config.HasGatewayA1 {
...
func (c *c) getGatewayClass(className string) (*gatewayv1alpha2.GatewayClass, error) {
	if !c.config.HasGateway {
...
func (c *c) GetGatewayA1List() ([]*gatewayv1alpha1.Gateway, error) {
	if !c.config.HasGatewayA1 {
		return nil, errGatewayA1Disabled
	}
...
func (c *c) GetHTTPRouteA1List(namespace string, match map[string]string) ([]*gatewayv1alpha1.HTTPRoute, error) {
	if !c.config.HasGatewayA1 {
		return nil, errGatewayA1Disabled
	}
	...
	list := gatewayv1alpha1.HTTPRouteList{}
	err = c.client.List(c.ctx, &list, &client.ListOptions{LabelSelector: selector, Namespace: namespace})
	refList := make([]*gatewayv1alpha1.HTTPRoute, len(list.Items))
	for i := range list.Items {
...
func (c *c) GetGatewayMap() (map[string]*gatewayv1alpha2.Gateway, error) {
	if !c.config.HasGateway {
		return nil, errGatewayA2Disabled
	}
...
func (c *c) GetHTTPRouteList() ([]*gatewayv1alpha2.HTTPRoute, error) {
	if !c.config.HasGateway {
		return nil, errGatewayA2Disabled
	}
